...
\begin{center}
  ...
  B\'ar\'any, Kolossv\'ary, and Troscheit
\end{center}
...
point. / Comment 25: Page 23, to apply Lemma 4.1 we need the IFS to have no exact overlaps,
however you pick an arbitrary IFS. }

\response{Thank you for bringing this to our attention. We believe the referee is referring to Theorem~1.4,
where Lemma~4.1 is applied in the density part of the proof. (Theorem 1.12 itself does not use Lemma
4.1, and the page reference points to page 23, where Theorem 1.4 is proven.)
...
All internal cross-references used \texttt{\textbackslash cref\{\}}, and there was a bug in the
...
\part*{Additional changes since submission}
...
\begin{itemize}
  \item  We believe the analyticity condition is a natural and essential choice: it is required by the methods
    of Rapaport, as well as our own. Relaxing to less regularity, even just $C^\infty$,
    poses separate challenges and invalidates some results. 
    In the introduction, we now include a short discussion on the choice of analytic maps and
    references to phenomena occurring in the irregular non-analytic case.

  \item In Theorem 2.3, we corrected the second statement to accurately reflect that having
    a common fixed point is the right restatement of Theorem 1.12.
  ...
  \item As suggested by Amir Algom, we added a paragraph on the uniform non-integrability (UNI) condition of Dolgopyat and
  ...
  \item References in the bibliography have been updated where necessary.
\end{itemize}
