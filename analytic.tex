...
\begin{theorem}\label{thm:SubConjugation}
  Any $\Phi\in\mathfrak{S}_N$ is conjugated to another analytic IFS which has at least one affine map. 
  Moreover, $\Phi$ is 
  \begin{enumerate}
	...
	H_{\bi}(x) \equiv H_{\bj}(x)
      \]
      for all $x\in[0,1]$;
    \item sub-conjugated to a self-similar IFS if and only if there exist distinct $\bi,\bj\in\Sigma_*$
...
We remark that analytic IFS with an exact overlap are sub-conjugated to self-similar IFSs, see
{\color{red} Link to relevant section ??}
...
$\mathcal{B}_{\epsilon}$ such that $f\colon I \to \mathbb{R}$.
We equip $\mathcal{C}_\epsilon^\omega([0,1])$ with the supremum norm $\|.\|_\infty$ and note that it is a complete
and separable metric space.
Given an analytic IFS $\Phi=(f_i)_{i=1}^N\in\mathfrak{S}_N$, we `lift' each map $f_i$ to an operator $F_i:
...
considered an affinity map and $\Phi^*$ as self-affine IFS on $\mathcal{C}_\epsilon^\omega$.
Indeed, $F_i$ is a
translation of the linear operator $h\mapsto f'_i(x)\cdot h(f_i(x))$. 
With this interpretation the function $H_{\bi}(x)$ introduced
...
\begin{theorem}\label{thm:DualSSC}
  An IFS $\Phi\in\mathfrak{S}_N$ satisfies the SESC if its dual IFS $\Phi^*$ satisfies the SSC (which is
...
strict contraction in the supremum norm since $\|F_ig-F_ih\|_{\infty}\leq \|f'_i\|_{\infty}\cdot
...
in~\cref{eq:H_i(x)}. This motivates us to call $H_{\bi}(x)$ the \emph{dual natural projection}.
We first justify calling $\Phi^*$ an IFS by showing that it has an
attractor. By convention, if $A\subset \mathcal{C}^{\omega}_\epsilon([0,1])$, then
\begin{equation*}
...
\end{equation*} 
\end{lemma} 
This can be proved analogously to \cite{Hutchinson_Attractor_81} by noting that all $F_i$ are strict
contractions with respect to the supremum norm and that $\mathcal{C}_\epsilon^\omega([0,1])$ is
complete and separable. We omit details.
...
$$(k,K)\coloneqq \big\{g\in\mathcal{C}_\epsilon^{\omega}([0,1]):\, k<g(x)<K\big\}.$$
