...
% Fix cleveref type detection for shared theorem counter.
% \refstepcounter{theorem} is used for all environments, so cleveref
% thinks everything is a "theorem". We patch \label within each environment
% to explicitly specify the correct type.
\makeatletter
\newcommand{\crefpatch}[1]{%
  \AtBeginEnvironment{#1}{%
    \let\orig@label\label
    \renewcommand{\label}[1]{\orig@label[#1]{##1}}%
  }%
}
\crefpatch{corollary}
\crefpatch{proposition}
\crefpatch{lemma}
\crefpatch{claim}
\crefpatch{conjecture}
\crefpatch{problem}
\crefpatch{question}
\crefpatch{definition}
\crefpatch{remark}
\crefpatch{example}
\makeatother
%\usepackage{refcheck}
...
Rapaport pointed out that the analyticity assumption is crucial for the proof of his main result.
In fact, his methods fail even for $C^\infty$-maps.
...
Just like in the case of Rapaport, the assumption of analyticity is crucial for our methods as well,
since at various places in the proof we need to transition from local information to global bounds.
To ensure sufficient regularity, the analyticity is a natural condition, see for example
\cite{AlgomEtal_NonLinHyperbolicIFS} and \cite[Example~7.7]{AngelevskaKaenmakiTroscheit2020}.


\paragraph{Acknowledgements.} The authors would like to thank Amir Algom for pointing out various
useful references on conjugation of analytic IFSs to self-similar IFSs. We further thank Ga\'etan
Leclerc and Tuomas Sahlsten for further discussions and the anonymous
...
In the setting of M\"obius IFSs satisfying the SESC,
...
our example below.
A natural open question is whether Usuki's result extends to analytic self-conformal
systems.
...
\begin{theorem}\label{thm:DualConj}
	...
	only if there exist
	$\bi,\bj\in\Sigma_*$ of the same length such that $F_{\bi}$ and $F_{\bj}$ have the same
	fixed point.
\end{theorem}
...
By \cref{thm:DualSSC}, it is enough to show $\{\Phi\in\mathfrak{S}_N:\Phi^*\text{
...
Let us begin the proof of \cref{thm:SubConjugation} with the following observation, which is
essentially a one-dimensional analytic instance of the Poincar\'e--Siegel linearisation theorem; see
\cite[Theorem~2.8.2]{KatokHasselblatt}??. We give a self-contained proof tailored to our setting,
since we need the explicit form of the conjugating map~$g$ in terms of $\hat{H}_f$.
