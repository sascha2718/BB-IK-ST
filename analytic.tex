...
\newcommand{\rfootnote}[1]{\footnote{\color{green!60!black}#1}}
...
\begin{abstract}
In a recent article, Rapaport showed that there is no dimension drop for
...
dimension. Similar questions can be asked for the measure, see Cawley and Mauldin
...
\paragraph{Acknowledgements.} The authors would like to thank Amir Algom for pointing out various
useful references on conjugation of analytic IFSs to self-similar IFSs. We also thank the anonymous
referees for their careful reading and helpful comments.

BB acknowledges support from grant NKFI~K142169, and grant NKFI
...
\begin{enumerate}[label=(\Alph*)]
  \item\label{it:a} $f$ is complex analytic in the open $\epsilon$ complex neighbourhood
    $\mathcal{B}_{2\epsilon}$ of  $I$, where $\mathcal{B}_{\delta}\coloneqq\{z\in\mathbb{C}:\exists
    x\in I,\, |z-x|<\delta\}$,
...
In the setting of M\"obius IFSs satisfying the strongly Diophantine condition,
Usuki~\cite{Usuki_LqMobius25arXiv} proved a dichotomy for the $L^q$ spectrum: either
the $L^q$ dimension takes the expected value for all $q>1$, or the spectrum eventually becomes
linear. A key observation is that the latter case occurs whenever two maps in the IFS share a
common fixed point in the attractor, which is precisely the mechanism behind the dimension drop in
our example below.
...
\begin{equation}\label{eq:maxmincontract}
  c_{\min} := \inf_{\substack{x\in\overline{\mathcal{B}_{\epsilon}} \\ i\in\mathcal{I}}}
  |f_i'(x)| \quad\text{and}\quad
  c_{\max}:=\sup_{\substack{x\in\overline{\mathcal{B}_{\epsilon}} \\ i\in\mathcal{I}}}
  |f_i'(x)|.
\end{equation}
We note that by \cref{it:c} and compactness, $0<c_{\min} \leq c_{\max} <1$.
...
every non-degenerate closed interval $J\subseteq[0,1]$
...
\begin{align*}
	...
	\sum_{n=2}^{\infty}\; \sup_{x\in[0,1]} \big| f_{\bi_{n-1}^{1}}'(x) \big| \cdot \sup_{x\in[0,1]}
	\left| \frac{f_{i_n}''(x)}{f_{i_n}'(x)} \right| \\
	&\geq \alpha -  2\frac{c_{\max}}{1-c_{\max}}\cdot \beta >0
\end{align*}
by our assumption. Hence, $H_{\bi}\not\equiv H_{\bj}$ and using the analyticity of the maps (which
will be verified later in \cref{thm:H_iAnalytic}) \cref{eq:posallint} follows.
...
while $f_3$ has fixed point at $x=1$. Since the maps are polynomials, for $\epsilon>0$
sufficiently small we have $c_{\max}<1/5$. Moreover,
\begin{equation*}
...
\frac{1}{2} .
\end{equation*}
Since $c_{\max}<1/5$ implies $2\beta\cdot c_{\max}/(1-c_{\max})<1/2=\alpha$, it follows from~\cref{prop:example} that $\Phi$ satisfies the
...
\rfootnote{------- Some thoughts. New `theorem' about polynomials (IK thinks sections 1.2.2 and
1.2.3 should be switched and proof should probably be separate section at the end of
paper)-----------}

There is something we need to consider. We have the condition in \cref{eq:H_iSSC} which is
sufficient to show SESC. We should remark that it is far from necessary because whenever there
exists two finite words $\bi\neq\bj$ such that $f_{\bi_{|\bi|}^{1}}(x) = \lambda\cdot
f_{\bj_{|\bj|}^{1}}(x) + t$ for some $|\lambda|\neq 0$ and $t\in\mathbb{R}$ (it is saying something
like two different compositions of functions are a scaled and translated copy of each other), then
$H_{\bi}(x)\equiv H_{\bj}(x)$ for all $x\in[0,1]$, so in particular \cref{eq:H_iSSC} does not hold.
However, the IFS could still very easily satisfy even the SSC. So here is some kind of new
``separation condition'' which is much weaker than exact overlaps, but can also fail for systems
with SSC. 

So $H$ can not distinguish between two such compositions. What could be a refinement? It seems that
this is the problem we run into in our theorem attempt for polynomials. We could introduce a
condition something like for every fnite length $\bi\neq\bj$ and every $|\lambda|\neq 0$ and
$t\in\mathbb{R}$ we assume that there exists an $x\in[0,1]$ such that $f_{\bi_{|\bi|}^{1}}(x) \neq
\lambda\cdot f_{\bj_{|\bj|}^{1}}(x) + t$. {\color{red}How checkable is this condition?? For example,
for polynomial IFS, if this is true on the first level, then is it true for all compositions?} It
does not seem at all that if $f_{\bi_{|\bi|}^{1}}(x) = \lambda\cdot f_{\bj_{|\bj|}^{1}}(x) + t$,
then there is a conjugating function that conjugates both $f_{\bi_{|\bi|}^{1}}$ and
$f_{\bj_{|\bj|}^{1}}$ simultaneously to a linear function. {\color{red} The theorem will definitely
need to be restated.}

\begin{theorem}
Let $\Phi=(f_i)_{i\in\mathcal{I}}\in\mathfrak{S}_N$ be an IFS consisting of polynomials
\begin{equation*}
f_i(x) = \sum_{n=0}^{N_i} a_n^{(i)}\cdot x^n
\end{equation*}
that is not sub-conjugated to a self-similar IFS, where $\mathrm{deg} f_i=N_i$. Assume that for any
$\bi\neq\bj\in\Sigma_\ast$ with $|\bi|=|\bj|$ and for every $|\lambda|\neq 0$ and $t\in\bbR$ there
exists $x\in[0,1]$ such that $f_{\bi}(x)\neq \lambda f_{\bj}(x) +t$.   Then $\Phi$ satisfies the
SESC. 
%there is a dichotomy:
%\begin{equation*}
%\text{either }\quad \Phi \text{ } \quad \text{ or } \quad \Phi \text{ has exact overlaps}.
%\end{equation*}
\end{theorem}

 
\begin{proof}
In light of \cref{thm:main}, we calculate the difference $|H_{\bi}(x) - H_{\bj}(x)|$ for all
distinct $\bi,\bj \in\Sigma\cup\Sigma_*$ with $|\bi|=|\bj|$. First assume $|\bi|=|\bj|<\infty$. Then
\cref{eq:H=f''/f'} implies that
\begin{equation*}
|H_{\bi}(x) - H_{\bj}(x)|
= \left| \frac{f''_{\bi_{|\bi|}^1}(x)}{f'_{\bi_{|\bi|}^1}(x)} -
\frac{f''_{\bj_{|\bj|}^1}(x)}{f'_{\bj_{|\bj|}^1}(x)} \right|
= \left| \frac{f''_{\bi_{|\bi|}^1}(x)\cdot f'_{\bj_{|\bj|}^1}(x) - f''_{\bj_{|\bj|}^1}(x) \cdot
f'_{\bi_{|\bi|}^1}(x)}{ f'_{\bi_{|\bi|}^1}(x) \cdot f'_{\bj_{|\bj|}^1}(x)} \right|.
\end{equation*}
The composition
\begin{equation*}
f_{\bi_{|\bi|}^1}(x) = \sum_{k=0}^{N_{\bi}} a_k^{(\bi)}\cdot x^k
\end{equation*}
is also a polynomial of degree $N_{\bi}=\prod_{k=1}^{|\bi|}N_{i_k}$ with $|a_1^{(\bi)}|\neq0$ since
$|f'_{\bi_{|\bi|}^1}(x)|\in(0,1)$ for all $x\in[0,1]$. During the proof, slightly abusing notation,
we simply write $f_{\bi}=f_{\bi_{|\bi|}^1}$. Therefore, both $f''_{\bi}\cdot f'_{\bj}$ and
$f'_{\bi}\cdot f''_{\bj}$ are polynomials of degree $N_{\bi}+N_{\bj}-3$. More specifically,
\begin{equation*}
f''_{\bi}(x)\cdot f'_{\bj}(x) = \sum_{k=0}^{N_{\bi}+N_{\bj}-3} \Big( \sum_{\ell=0}^{k}
(\ell+2)(\ell+1)a_{\ell+2}^{(\bi)}\cdot (k-\ell+1)a_{k-\ell+1}^{(\bj)} \Big) \cdot x^k,
\end{equation*}
where we set $a_k^{(\bi)}=0$ for all $k>N_{\bi}$ and similarly $a_k^{(\bj)}=0$ for all $k>N_{\bj}$.

Since $\Phi$ is not sub-conjugated to a self-similar IFS, either $f''_{\bi}(x)\cdot f'_{\bj}(x) =
f'_{\bi}(x)\cdot f''_{\bj}(x)$ for at most  $N_{\bi}+N_{\bj}-3$ points $x\in[0,1]$, in which case
$\sup_{x\in[0,1]} |H_{\bi}(x) - H_{\bj}(x)| > 0$ automatically holds; or $f''_{\bi}(x)\cdot
f'_{\bj}(x) = f'_{\bi}(x)\cdot f''_{\bj}(x)$ for every $x\in[0,1]$, in which case the corresponding
coefficients of the two polynomials must be the same, \ie for every $0\leq k\leq N_{\bi}+N_{\bj}-3$,
\begin{equation}\label{eq:EqualCoeffs}
\sum_{\ell=0}^{k} (\ell+2)(\ell+1)a_{\ell+2}^{(\bi)}\cdot (k-\ell+1)a_{k-\ell+1}^{(\bj)}
= \sum_{\ell'=0}^{k} (\ell'+2)(\ell'+1)a_{\ell'+2}^{(\bj)}\cdot (k-\ell'+1)a_{k-\ell'+1}^{(\bi)}.
\end{equation}
We show that in this case $f_{\bi_{|\bi|}^1}(x) = \frac{a_1^{(\bi)}}{a_1^{(\bj)}}\cdot
f_{\bj_{|\bj|}^1}(x) +a_0^{(\bi)} -\frac{a_1^{(\bi)}}{a_1^{(\bj)}}\cdot a_0^{(\bj)}$.
\begin{claim}\label{thm:Claim}
If $\Phi$ is not sub-conjugated to a self-similar IFS and \cref{eq:EqualCoeffs} holds, then
$N_{\bi}=N_{\bj}$, moreover,  $a_m^{(\bi)}=\frac{a_1^{(\bi)}}{a_1^{(\bj)}}\cdot a_{m}^{(\bj)}$ for
every $1\leq m\leq N_{\bi}$.
\end{claim}
\begin{proof}[Proof of \cref{thm:Claim}]
Without loss of generality we assume that $N_{\bj}\geq \max\{N_{\bi},2\}$ and introduce
$\gamma=a_1^{(\bi)}/a_1^{(\bj)}\neq 0$. The proof goes by induction on $m$. From $k=0$ in
\cref{eq:EqualCoeffs} we see that $2 a_2^{(\bi)}a_1^{(\bj)}=2 a_2^{(\bj)}a_1^{(\bi)}$. Since
$a_2^{(\bj)}\neq0$, we obtain that $a_2^{(\bi)}\neq 0$, hence, $N_{\bi}\geq 2$. Furthermore,
$a_2^{(\bi)}=\gamma a_2^{(\bj)}$ which confirms the claim for $m=2$.

For the induction hypothesis we assume that for some $M< N_{\bj}$ the following holds:
$a_m^{(\bi)}=\gamma a_{m}^{(\bj)}$ for every $1\leq m\leq M$. We need to prove that
$a_{M+1}^{(\bi)}=\gamma a_{M+1}^{(\bj)}$. Using \cref{eq:EqualCoeffs} for $k=M-1$, we get 
\begin{multline*}
M(M+1)\cdot a_{M+1}^{(\bi)}a_1^{(\bj)}+\sum_{\ell=0}^{M-2} (\ell+2)(\ell+1)a_{\ell+2}^{(\bi)}\cdot
(k-\ell+1)a_{k-\ell+1}^{(\bj)} \\
= M(M+1)\cdot a_{M+1}^{(\bj)}a_1^{(\bi)}+ \sum_{\ell'=0}^{M-2}
(\ell'+2)(\ell'+1)a_{\ell'+2}^{(\bj)}\cdot (k-\ell'+1)a_{k-\ell'+1}^{(\bi)}.
\end{multline*}
The two sums cancel each other out term by term because of the induction hypothesis, which leaves us
with $a_{M+1}^{(\bi)}a_1^{(\bj)}=a_{M+1}^{(\bj)}a_1^{(\bi)}$. Since $M<N_{\bj}$, we have
$a_{M+1}^{(\bj)}\neq0$, which implies that $a_{M+1}^{(\bi)}\neq 0$ and in particular
$a_{M+1}^{(\bi)}=\gamma a_{M+1}^{(\bj)}$. 
\end{proof}
Having established \cref{thm:Claim}, we immediately see that
\begin{equation*}
	f_{\bi_{|\bi|}^1}(x) = a_0^{(\bi)}+\sum_{k=1}^{N_{\bi}} a_k^{(\bi)}\cdot x^k = a_0^{(\bi)} +
	\frac{a_1^{(\bi)}}{a_1^{(\bj)}}\cdot  \sum_{k=1}^{N_{\bj}} a_k^{(\bj)}\cdot x^k =
	\frac{a_1^{(\bi)}}{a_1^{(\bj)}}\cdot f_{\bj_{|\bj|}^1}(x) +a_0^{(\bi)}
	-\frac{a_1^{(\bi)}}{a_1^{(\bj)}}\cdot a_0^{(\bj)},
\end{equation*}
which contradicts our assumption. Hence, it is not possible that $f''_{\bi}(x)\cdot f'_{\bj}(x) =
f'_{\bi}(x)\cdot f''_{\bj}(x)$ for every $x\in[0,1]$.

\rfootnote{Is it possible to deduce something more concrete about $a_0^{(\bi)}, a_0^{(\bj)}$ and $\gamma$??}
\rfootnote{Still missing argument when $|\bi|=|\bj|=\infty$.}
\rfootnote{This is simply not true} Since $\Phi$ is not sub-conjugated to a self-similar IFS, it
follows that $\gamma=1$ and that $a_0^{(\bi)}=a_0^{(\bj)}$. Thus, we have shown that $\Phi$ has the
exact overlap $f_{\bi_{|\bi|}^1} \equiv f_{\bj_{|\bj|}^1}$.

\end{proof}
\rfootnote{------- End of proof for new theorem about polynomials -----------}
...
\cite{AlgomHertzWang23,AlgomEtal_LogFourierDecaySelfConf_JLMS,BakerBanaji_PolyFourierDecay} and \cite[Corollary 1.2 part
...
the function $H_{\bi}$ introduced in~\cref{eq:H_i(x)}. We point out that Algom, Rodriguez
Hertz, and Wang \cite[Claim 6.1]{AlgomHertzWang23} already gave a dichotomy stating that 
either (a) there is a analytic conjugation to a self-similar IFS or (b) that any such conjugation
cannot even be $\mathcal{C}^2$. Our result can be considered
complementary, as we give a checkable condition when we are conjugated to a self-similar system.
...
A strongly related condition is that of uniform non-integrability (UNI) introduced by
Dolgopyat and Chernov, which was used in the IFS context by Baker and Sahlsten to determine Fourier
decay of measures, see \cite{BakerSahlsten23} and references therein.
In our terminology, the UNI condition is equivalent to the existence of 
$\bi,\bj\in\Sigma$ such that 
$\inf_{x\in I} |H_{\bi}(x) -H_{\bj}(x)| >0$, which is clearly stronger than the negation of
\cref{it:conj1} in \cref{thm:SubConjugation}.


\begin{remark}
  Let us note that in \cref{it:conj1} one only needs to check that $H_{i^\infty}(x)\equiv
  H_{j^\infty}(x)$ for all $i,j \in\mathcal{I}$ and $x\in I$.
  This may not be clear at this point, but will be immediate after the introduction of the dual IFS
  and \cref{thm:DualConj}.
\end{remark}

We return to the discussion of the dimension drop conjecture. Recall
...
\begin{proof}
  ...
  $\left|\frac{f_i'''(x)f_i'(x)-(f_i''(x))^2}{(f_i'(x))^2}\right|<C$ for every $i\in\mathcal{I}$ and
  ...
  \begin{equation}\label{eq:tec3}
    \sup_{x\in[0,1]}|f_{\bi}'(x)-g_{\bi}'(x)|\leq |\bi|\cdot c_{\max}^{|\bi|-1}\frac{C+1}{1-c_{\max}}d_2(\Phi,\Psi).
  \end{equation}
...
&\leq\left(\frac{|\bi|\cdot C(C+1)+C}{1-c_{\max}}+\frac{c_{\max}+C}{c_{\min}^2}\right)c_{\max}^{|\bi|-1}d_2(\Phi,\Psi).
  \end{align*}
...
y_{\ell}}\big(H_{\bi}^{(k-2)},\dots,H_{\bi}(x)\big) \cdot H_{\bi}^{(\ell+1)}(x).
\]
...
E_{|B|-1}
\leq \sum_{\pi\in\Pi_{k+1}} \frac{D_{|\pi|}\cdot \prod_{B\in\pi}E_{|B|-1}}{1-c_{\max}^{|\pi|}}
\]
...
\begin{equation*}
  ...
  E_2\cdot c_{\max}^{|\bi|}
\end{equation*}
by the definition of $E_k$ in~\cref{eq:E_k}. Since $\eta_{n_\ell}\to 0$ super-exponentially, we may
assume $\eta_{n_\ell}<1/4$ for all $\ell$. Together with~\cref{thm:analyticity} these bounds imply
that for the particular choice of $\bi,\bj$ in~\cref{eq:ExpCondensation} we have
\begin{align*}
...
\[
  (\bi^{(r)},\bj^{(r)})=(i_1^{(r)},\ldots,
i_n^{(r)};j_1^{(r)},\ldots,j_n^{(r)}),
\]
where $r=1,\ldots,\#\mathcal{B}_n$. The points are constructed
...
We continue by induction. Assume that $x_{(\bi^{(1)},\bj^{(1)})},\ldots,x_{(\bi^{(r)},\bj^{(r)})}$
have already been constructed (for some $r\geq 1$) so that \cref{it:lla,it:llb,it:llc} all hold. The set
$\bigcup_{m=1}^r \widehat{\mathcal{O}}(x_{(\bi^{(m)},\bj^{(m)})})$ is finite, where we use the
shorthand  $\widehat{\mathcal{O}}(x_{\bi,\bj})\coloneqq\big(\mathcal{O}_{\bi}(x_{\bi,\bj})\cup
\mathcal{O}_{\bj}(x_{\bi,\bj})\big)$. Then the set
\begin{equation*}
A_r\coloneqq \bigcup_{\ell=0}^n \bigg( \big(f_{(\bi^{(r+1)})_{\ell}^1}\big)^{-1} \Big(
\bigcup_{m=1}^r \widehat{\mathcal{O}}(x_{(\bi^{(m)},\bj^{(m)})}) \Big) \cup
\big(f_{(\bj^{(r+1)})_{\ell}^1}\big)^{-1} \Big( \bigcup_{m=1}^r
...
If $(\bi^{(r+1)},\bj^{(r+1)})$ is not
bad, then using that $A_r$ is finite and the continuity of the maps one can choose $\hat
x_{(\bi^{(r+1)},\bj^{(r+1)})}\in(0,1)\setminus A_r$ for which \cref{it:lla}
holds. By continuity of the maps, there exists a small neighbourhood of $\hat
x_{(\bi^{(r+1)},\bj^{(r+1)})}$ in $(0,1)\setminus A_r$ where \cref{it:lla} still holds, and by the same
argument as before can be used to pick a $x_{(\bi^{(r+1)},\bj^{(r+1)})}$ from this small
neighbourhood for which \cref{it:lla,it:llb,it:llc} all hold. If $(\bi^{(r+1)},\bj^{(r+1)})$ is bad, then
analyticity and the no exact overlaps assumption imply again the existence of
$x_{(\bi^{(r+1)},\bj^{(r+1)})}\in(0,1)\setminus A_r$ that satisfies \cref{it:llb} which completes the
induction.
\end{proof}
...
Let $\delta>0$ and $\Phi=(f_i)_{i\in\mathcal{I}}\in\mathfrak{S}_N$ be arbitrary but fixed. We may
assume that $\Phi$ has no exact overlaps, since otherwise we can make an arbitrarily small
perturbation to remove them. Choose
...
Recall $\mathcal{O}_{\bi}(x)= \{x,f_{i_1}(x), f_{i_2i_1}(x),\ldots,f_{i_{|\bi|}\ldots i_1}(x)\}$.
For every $i\in\mathcal{I}$, we define a partition of $\bigcup_{(\bi,\bj)\in\mathcal{B}_n}
\big(\mathcal{O}_{\bi}(x_{\bi,\bj})\cup
...
\begin{itemize}
	...
	\item $x_{\bi,\bj}\in\mathcal{Z}_{i_1}$ and $x_{\bi,\bj}\in\mathcal{Y}_{j_1}$ if
	  ...
	  \]
	\item $x_{\bi,\bj}\in\mathcal{Z}_{i_1}$ and $x_{\bi,\bj}\in\mathcal{Z}_{j_1}$ if
	  $(\bi,\bj)\in\mathcal{B}_n$ is not bad;
	\item $y\in\mathcal{Z}_{i_1}$ and $y\in\mathcal{Z}_{j_1}$ for every
	  $y\in\bigcup_{(\bi,\bj)\in\mathcal{B}_n} \big(\mathcal{O}_{\bi}(x_{\bi,\bj})\cup
...
Let $f\in\mathcal{S}_\epsilon^\omega([0,1])$ be arbitrary but fixed. Along the lines of
\cref{eq:H_i(x)}, let us define the map
\begin{equation}\label{eq:H}
\hat{H}_f(x)\coloneqq\sum_{k=0}^\infty\frac{f''}{f'}(f^{\circ k}(x))\cdot (f^{\circ k})'(x),
\end{equation}
where $f^{\circ k}$ denotes the self-composition of $f$ $k$-times. Analogously to the proof of
\cref{thm:H_iAnalytic}, $\hat{H}\in\mathcal{C}^\omega_\epsilon([0,1])$. 
...
\begin{proof}[Proof of~\cref{thm:SubConjugation}]
	...
	The assertion \cref{it:conj2} of \cref{thm:SubConjugation} follows by
	applying \cref{it:conj1} to the sub-IFS $(f_{\bi},f_{\bj})$, so we finish the proof by
	showing \cref{it:conj1}.
