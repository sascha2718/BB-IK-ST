...
Let
\begin{equation*}
  c_{\max}:=\sup_{\substack{x\in[0,1] \\ i\in\mathcal{I}}} \left|f_i'(x)\right|\text{ and
  }\beta:=\sup_{\substack{x\in[0,1] \\ i\in\mathcal{I}}} \left| \frac{f_i''(x)}{f_i'(x)} \right|.
\end{equation*}
Then $0<c_{\max}<1$, $\beta>0$ and if $\alpha > 2\beta\cdot c_{\max}/(1-c_{\max})$, we have
$
\sup_{x\in[0,1]} |H_{\bi}(x) - H_{\bj}(x)| > 0 
$
...
\begin{proof}
  Let 
  $$
  c_{\min}:=\inf_{\substack{x\in[0,1] \\ i\in\mathcal{I}}} \left|f_i'(x)\right|.
  $$
  By a standard compactness argument, $0<c_{\min}\leq c_{\max}<1$ and $\beta>0$.
