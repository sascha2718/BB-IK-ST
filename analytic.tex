...
13–15, Hungary}\par\nopagebreak
  ...
  \textit{E-mail address}, S.~TroschRight\cref{it:la}$\Leftarrow$\cref{it:lb} \texttt{sascha.troscheit@math.uu.se}
}}
...
exists for the $L^q$ dimension of self-similar measures. This does not extend to
self-conformal measures in general. In the example below we give an explicit example of an IFS that
satisfies the SESC and a lengthy calculation (omitted) shows that measures in a small neighbourhood
of the natural measure exhibit a dimension drop for large $q$.
